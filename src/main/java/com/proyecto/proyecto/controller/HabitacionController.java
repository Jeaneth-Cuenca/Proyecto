...
import com.proyecto.proyecto.Repository.HabitacionRepository;
...
@Tag(name = "Habitación", description = "Operaciones relacionadas con habitaciones")
public class HabitacionController {
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<mensaje> deleteHabitacion(@PathVariable Long id) {
        if (habitacionRepository.existsById(id)) {
            habitacionRepository.deleteById(id);
            logger.info("Habitación eliminada con ID {}", id);
            return ResponseEntity.ok(new mensaje("Habitación eliminada correctamente", "success"));
        } else {
            logger.warn("No se encontró habitación con ID {}", id);
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(new mensaje("Habitación no encontrada", "error"));
        }
    }
