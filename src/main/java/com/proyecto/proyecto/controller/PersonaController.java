...
public class PersonaController {
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<mensaje> deletePersona(@PathVariable Long id) {
        if (personaRepository.existsById(id)) {
            personaRepository.deleteById(id);
            return ResponseEntity.ok(new mensaje("La persona fue eliminada correctamente", "success"));
        } else {
            logger.warn("No se encontró persona con ID {}", id);
            return ResponseEntity.ok(new mensaje("La persona no fue encontrada", "not_found"));
        }
    }
